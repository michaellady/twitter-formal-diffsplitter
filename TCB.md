...
| `crates/diffsplitter/src/scoreboard.rs` | `GET /scoreboard` (HTML), `GET /scoreboard.json` — rolling cross-impl conformance summary: total counts, divergence rate over 1h/24h/7d windows, inline-SVG severity bar chart (24h), top-10 diverging paths, last-critical pointer; greys out the rate panels when shadow is degraded (K13). | Read-only renderer over the same `diffs`/`write_queue`/`failed_writes` tables. A bug here changes the topline "are we converged?" answer that operators see first; an over-stated rate cries wolf, an under-stated rate hides regressions. The "requests" denominator is approximated as `diffs + writes` (no per-request audit row exists yet) — this is conservative (overstates rather than hides), and the page footer documents the tradeoff. No auth; no `<script>`; HTML escaped at render time; page weight ≤ 50KB enforced by integration test. | `cargo test` (`tests/scoreboard.rs`): HTML returns 200 with all required sections, JSON round-trips through SQLite, divergence-rate window math is correct on a seeded DB, empty DB returns sensible zeros, degraded shadow shows the banner. |
...
- **Conformance scoreboard (read-only HTML):** `crates/diffsplitter/src/scoreboard.rs`
